...
const LEGACY_POST_SLUGS = {
  ...
  'post-calculate-expression': 'how-to-calculate-expression-number',
  'post-calculate-soul-urge': 'how-to-calculate-soul-urge-number',
  'post-calculate-personality': 'how-to-calculate-personality-number',
  'post-hearts-desire': 'hearts-desire-vs-soul-urge-numerology',
  'post-numerology-chart': 'numerology-chart-explained',
  'post-beginners': 'numerology-for-beginners',
  'post-name-numerology': 'name-numerology',
  'post-master-44': 'master-number-44-numerology',
  'post-y-vowel': 'is-y-a-vowel-in-numerology',
  'post-destiny-vs-expression': 'destiny-number-vs-expression-numerology',
  'post-life-calling': 'life-calling-number-numerology',
  'post-karmic-debt': 'karmic-debt-numbers-13-14-16-19',
  'post-achievement': 'achievement-number-birthday-numerology',
  'post-name': 'birth-name-vs-known-name-numerology',
  'post-name-change': 'name-change-numerology-simulation',
  'post-compatibility': 'numerology-compatibility',
  'post-1111': '1111-angel-number-meaning',
  'post-444': '444-angel-number-meaning',
  'post-maturity': 'maturity-number-vs-life-calling',
  'post-career': 'career-numerology',
  'post-compounds': 'compound-numbers-numerology',
  'post-what-is-numerology': 'what-is-numerology',
  'post-222': '222-angel-number-meaning',
  'post-555': '555-angel-number-meaning',
  'post-777': '777-angel-number-meaning',
};
  ...
  if (name === 'calculator') {
    ...
    },
    {
      ...
      mainEntity: [
        ...
        _faqEntity('What is Source Decoder?', 'A community for learning the system — $11/month. Weekly group calls, member chat, and deep dives into frequencies and compounds.'),
...
function initHomeStickyCta() {
  ...
  function updateSticky() {
    ...
    if (document.documentElement.classList.contains('hp-snapping')) {
      var faq = document.querySelector('.hp-faq-section.is-current');
...
function enableHomeSnap() {
  ...
  function ensureFooter() {
    footerHost = document.getElementById('include-footer');
  ...
  function footerTop() {
    return footerHost ? footerHost.offsetTop : 0;
  }

  function footerTarget() {
    return Math.min(footerTop(), maxScroll());
  }

  function inFooter() {
    if (!footerHost) return false;
    return scrollY() >= footerTarget() - 32;
  }
  ...
  function syncDots() {
    ...
    var existing = nav.querySelectorAll('.hp-snap-dot');
    if (existing.length === count) return;
    Array.prototype.forEach.call(existing, function (dot) { dot.parentNode.removeChild(dot); });
  ...
  function setProgress(i, footerActive) {
    index = i;
    atFooter = !!footerActive;
    sections.forEach(function (sec, n) {
      sec.classList.toggle('is-current', !footerActive && n === i);
    });
    if (nav) {
      Array.prototype.forEach.call(nav.querySelectorAll('.hp-snap-dot'), function (dot, n) {
        var on = footerActive ? n === sections.length : n === i;
    ...
    if (bar) {
      var steps = sections.length + (footerHost ? 1 : 0);
      var pos = footerActive ? steps : (i + 1);
      bar.style.width = '100%';
      bar.style.height = ((pos / steps) * 100) + '%';
    }
    var siteNav = document.querySelector('nav:not(.breadcrumb):not(.hp-snap-nav)');
    if (siteNav) siteNav.classList.toggle('nav-hero', i === 0 && !footerActive);
  ...
  function goToFooter() {
    ...
    locked = true;
    acc = 0;
    setProgress(sections.length - 1, true);
    var top = footerTarget();
    page.scrollTo({
      top: top,
      behavior: 'auto'
    });
  ...
  function advance(dir) {
    if (locked) return;
    if (dir > 0) {
      if (inFooter() || atFooter) return;
      if (index >= sections.length - 1) {
        goToFooter();
        return;
      }
    } else if (dir < 0 && (inFooter() || atFooter)) {
  ...
  function onWheel(e) {
    ...
    if (canScrollInner(e.target, e.deltaY) && !(index >= sections.length - 1 && e.deltaY > 0)) return;
    var need = snapNeed();
    if (inFooter() || atFooter) {
  ...
  function onTouchMove(e) {
    ...
    if (canScrollInner(e.target, dy) && !(index >= sections.length - 1 && dy > 0)) return;
    if (Math.abs(dx) > Math.abs(dy) + 6) return;
    if ((inFooter() || atFooter) && dy > 0) return;
    e.preventDefault();
  }
  ...
  function onTouchEnd(e) {
    ...
    if (onCarouselSwipe(e.target, dx, dy)) return;
    if (canScrollInner(e.target, dy) && !(index >= sections.length - 1 && dy > 0)) return;
    if (dy > 0 && (inFooter() || atFooter)) return;
    advance(dy > 0 ? 1 : -1);
  }
  ...
  function onScroll() {
    ...
    settleTimer = setTimeout(function () {
      ...
      if (inFooter() || atFooter) {
        ...
        if (scrollY() > sectionTop(sections[sections.length - 1]) + 48) {
          setProgress(sections.length - 1, true);
          return;
        }
      }
      ...
      else setProgress(nearest, false);
    }, 140);
  }
  ...
  function refresh() {
    ...
    ensureFooter();
    sections = Array.prototype.slice.call(page.querySelectorAll('.hp-snap'));
    syncDots();
    if (inFooter()) setProgress(sections.length - 1, true);
    else setProgress(currentFromScroll(), false);
  }
...
function disableHomeSnap() {
  if (_homeSnap) _homeSnap.destroy();
  else document.documentElement.classList.remove('hp-snapping');
}
