...
export default function FlowDetailPanel({ open, onClose, color = '#c9a84c', title, subtitle, icon, panelClassName = '', children }) {
      ...
      {open && (
        <>
          ...
          <motion.div
            key="panel"
            ref={panelRef}
            className={`flow-side-panel flow-side-panel--mobile${panelClassName ? ` ${panelClassName}` : ''}`}
            style={{
              '--flow-color':          color,
